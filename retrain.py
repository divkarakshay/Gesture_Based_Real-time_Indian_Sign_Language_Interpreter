...
def run_retrain():
  unparsed = []
  global FLAGS
  FLAGS = Namespace(image_dir='train_frames', output_graph='retrained_graph.pb', \
            intermediate_output_graphs_dir='/tmp/intermediate_graph/', intermediate_store_frequency=0, \
            output_labels='retrained_labels.txt', summaries_dir='training_summaries/long', \
            how_many_training_steps=400, learning_rate=0.01, testing_percentage=10, validation_percentage=10, \
            eval_step_interval=10, train_batch_size=10, test_batch_size=-1, validation_batch_size=10, \
            print_misclassified_test_images=False, bottleneck_dir='bottlenecks', final_tensor_name='final_result', \
            flip_left_right=False, random_crop=0, random_scale=0, random_brightness=0, \
            tfhub_module='https://tfhub.dev/google/imagenet/inception_v3/feature_vector/1', saved_model_dir='')
  print(FLAGS.__dict__)
  tf.app.run(main=retrain, argv=[sys.argv[0]] + unparsed)
